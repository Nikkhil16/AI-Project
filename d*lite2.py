...
while start != goal:
    ...
    if grid[next_step[0]][next_step[1]] == 0:
        start = next_step
        current = start
    if grid[next_step[0]][next_step[1]] == 0.5:
        if cost[next_step[0]][next_step[1]] == 100:
            start = next_step
            current = start
        else:
            km = km + m_heuristic(last)
            last = start
            cost[next_step[0]][next_step[1]] = 100
    ...
    if grid[next_step[0]][next_step[1]] == -0.5:
        if cost[next_step[0]][next_step[1]] == 0.02:
            start = next_step
            current = start
        else:
            km = km + m_heuristic(last)
            last = start
            cost[next_step[0]][next_step[1]] = 0.02
            neighbours = find_predecessors(next_step)
            for i in range(len(neighbours)):
                update_vertex(neighbours[i])
            compute_shortest_path()
    ...
    if grid[next_step[0]][next_step[1]] == 1:
        ...
        rhs[next_step[0]][next_step[1]] = float("inf")
        neighbours = find_predecessors(next_step)
        for i in range(len(neighbours)):
            update_vertex(neighbours[i])
        compute_shortest_path()
